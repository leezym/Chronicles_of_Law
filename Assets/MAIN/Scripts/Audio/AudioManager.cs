...
public class AudioManager : MonoBehaviour
{
    ...
    {
        StopTrack(channel, AudioBus.Music);
    }

    public void StopTrack(int channel, AudioBus bus, bool immediate = false)
    {
        AudioChannel c = TryGetChannel(channel, bus, createIfDoesNotExist: false);

        if (c == null)
            return;

        c.StopTrack(immediate);
    }
    ...
    public void StopTrack(string trackName, AudioBus bus, bool immediate = false)
    {
        if (string.IsNullOrWhiteSpace(trackName))
            return;
        ...
        foreach (var channel in dict.Values)
        {
            if (channel.activeTrack != null && channel.activeTrack.name.ToLower() == trackName)
            {
                channel.StopTrack(immediate);
    ...
    public AudioChannel TryGetChannel(int channelNumber, bool createIfDoesNotExist = false) => TryGetChannel(channelNumber, AudioBus.Music, createIfDoesNotExist);

    public AudioChannel TryGetChannel(int channelNumber, AudioBus bus, bool createIfDoesNotExist = false)
    {
        var dict = (bus == AudioBus.Ambience) ? ambienceChannels : channels;
