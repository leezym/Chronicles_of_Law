...
namespace COMMANDS
{
    public class CMD_DatabaseExtension_Audio : CMD_DatabaseExtension
    {
        ...
        public static void StopSong(string data)
        {
            ...
            {
                AudioManager.Instance.StopTrack(0, AudioBus.Music);
            }
            else
            {
                StopTrack(data, AudioBus.Music);
            }
        }
        ...
        public static void StopAmbience(string data)
        {
            if (string.IsNullOrWhiteSpace(data))
            {
                AudioManager.Instance.StopTrack(0, AudioBus.Ambience);
            }
            else
            {
                StopTrack(data, AudioBus.Ambience);
            }
        }

        public static void StopTrack(string data, AudioBus bus)
        {
            if (string.IsNullOrWhiteSpace(data))
            {
                AudioManager.Instance.StopTrack(0, bus);
                return;
            }

            if (int.TryParse(data, out int channel))
                AudioManager.Instance.StopTrack(channel, bus);
            else
                AudioManager.Instance.StopTrack(data, bus);
        }
    }
}
